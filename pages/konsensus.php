...
function getTopsisRanking($conn, $role)
{
    ...
    while ($r = mysqli_fetch_assoc($qA))
        $alternatif[$r['id_alternatif']] = $r;

    $matriks_x = [];
    $qN = mysqli_query($conn, "SELECT * FROM penilaian WHERE id_user='$id_user'");
    while ($r = mysqli_fetch_assoc($qN))
        $matriks_x[$r['id_alternatif']][$r['id_kriteria']] = $r['nilai'];

    $bobot = [];
    $qB = mysqli_query($conn, "SELECT * FROM bobot_user WHERE id_user='$id_user'");
    while ($r = mysqli_fetch_assoc($qB))
        $bobot[$r['id_kriteria']] = $r['nilai_bobot'];
    ...
    foreach ($kriteria as $idk => $k) {
        ...
        if ($k['atribut'] == 'benefit') {
            $sol_plus[$idk] = max($col);
            $sol_min[$idk] = min($col);
        } else {
            $sol_plus[$idk] = min($col);
            $sol_min[$idk] = max($col);
        }
    }
    $hasil = [];
...
<?php elseif ($is_ready && !$show_result): ?>
    <div class="alert-ready mb-4">
        ...
        <?php elseif ($role_sekarang == 'admin'): ?>
            <button class="btn btn-secondary" disabled>
                <i class="bi bi-lock-fill me-1"></i> Menunggu Eksekusi Kadispar
            </button>
        <?php else: ?>
            <button class="btn btn-secondary" disabled>Menunggu Eksekusi Kadispar</button>
        <?php endif; ?>
    </div>
<?php else: ?>
    <?php if ($is_vip): ?>
...
// 1. Ambil Juara 1
    ...
    $count_rank1 = 0;
    if (($rank_dm1[$id_juara] ?? 0) == 1)
        $count_rank1++;
    if (($rank_dm2[$id_juara] ?? 0) == 1)
        $count_rank1++;
    if (($rank_dm3[$id_juara] ?? 0) == 1)
        $count_rank1++;
    ...
    <div class="conclusion-card p-4">
        <div class="d-flex align-items-start gap-4">
            ...
            <div>
                <h5 class="fw-bold text-dark mb-2">Rekomendasi Terbaik: <span
                        class="text-primary"><?= $juara['nama_wisata'] ?></span></h5>
                <p class="text-muted mb-2">
                    Berdasarkan hasil perhitungan metode Borda Count dengan total perolehan
                    <strong><?= $juara['total_poin'] ?> Poin</strong>.
                </p>
                <div class="p-3 bg-white rounded border border-warning-subtle">
                    <strong class="d-block mb-1 text-warning-emphasis"><i class="bi bi-lightbulb me-1"></i> Alasan
                        Rekomendasi:</strong>
